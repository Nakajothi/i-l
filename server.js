...
async function callGeminiForMcqs(payload) {
  ...
  if (!jsonBlock) throw new Error('Gemini returned an empty response.');
  let parsed;
  try {
...
app.post('/api/teacher/mcqs/generate', authTeacher, async (req, res) => {
  try {
    ...
    const generated = await callGeminiForMcqs({
    ...
    res.json({ success: true, questions: questions.map((item, index) => sanitizeGeneratedMcq(item, index)) });
  } catch (err) {
